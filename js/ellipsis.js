...
jQuery.fn.dataTable.render.ellipsis = function ( cutoff, wordbreak, escapeHtml ) {
	...
	return function ( d, type, row ) {
		...
		dstr = $(d).text(); // cast numbers

		if ( dstr.length <= cutoff ) {
			return d;
		}
                ...
                if ($(d).length > 1 ) {
                        ...
                        newarry = [];
                        $(d).each(function() {
                            if (count + $(this).text().length <= cutoff) {
                                newarry.push( $(this) );
                                count += $(this).text().length;
                            } else if ( wordbreak ) {
                                newarry.push( $(this).text($(this).text().substr(0, cutoff-1)) );
                                return false;
                            } else {
                                return false;
                            }
                        ...
                        var shortened = "";
                        $(newarry).each(function() {
                            if ($(this).prop("outerHTML") != undefined) {
                                shortened += $(this).prop("outerHTML");
                            } else {
                                shortened += $(this).text();
                            }
                        });
                        return '<span class="ellipsis" title="'+esc(dstr)+'">'+shortened+'&#8230;</span>';
                }

		var shortened = $(d).text(dstr.substr(0, cutoff-1));

		// Find the last white space character in the string
		if ( wordbreak ) {
			shortened = shortened.text(shortened.text().replace(/\s([^\s]*)$/, ''));
		}
		...
		return '<span class="ellipsis" title="'+esc(dstr)+'">'+shortened.prop('outerHTML')+'&#8230;</span>';
	};
};
